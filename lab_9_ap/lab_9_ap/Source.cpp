...
int countNodes(const std::shared_ptr<Node>& root) {
    if (!root) {
        return 0;
    }
...
void printMenu() {
    ...
    std::cout << "9. The number of elements in this binary tree\n";
}
...
int main() {
    ...
    while (true) {
        ...
        case 9: {
            int nodeCount = countNodes(bst);
            std::cout << "elements in this tree: " << nodeCount << std::endl;
            break;
        }
        default:
